...
system.time({result = SimJoint::SJpearson(
  ...
  maxCore = 1, convergenceTail = 8, verbose = F)})
# user  system elapsed
# 0.30    0.00    0.29
...
# Max error magnitude is less than 5% of that from
# `SimMultiCorrData::rcorrvar()`:
# Min.          1st Qu.     Median       Mean    3rd Qu.       Max.
# -0.008336   -0.001321          0  -0.000329   0.001212    0.00339
# This able is reported in Step 4, correlation methods 1 or 2.
...
result = SimJoint::SJpearson(
  X = apply(marginals, 2, function(x) sort(sample(x, 1000, replace = T))),
  cor = Rey, errorType = "maxRela")
summary(round(as.numeric(cor(result$X) - Rey), 6))
# Min.           1st Qu.     Median       Mean    3rd Qu.       Max.
# -0.0037860 -0.0005100   0.0000000  0.0003257  0.0010320  0.0038440
...
# =============================================================================
\donttest{system.time({resultGenOrd = GenOrd::ordsample(
...
# function `EnvStats::simulateVector()` imposes rank correlations.
# =============================================================================
\donttest{rm(list = ls()); gc()
  cor.mat = matrix(c(1, 0.8, 0, 0.5, 0.8, 1, 0, 0.7, 0, 0, 1, 0.2, 0.5,
                     0.7, 0.2, 1), 4, 4)
  pareto.rns <- EnvStats::simulateVector(100, "pareto",
                                         list(location = 10, shape = 2), sample.method = "LHS",
                                         seed = 56)
  mat <- EnvStats::simulateMvMatrix(1000,
  ...
  marginals = cbind(sort(rnorm(1000, 10, 2)),
                    sort(EnvStats::rlnormAlt(1000, 10, 1)),
                    sort(rbeta(1000, 2, 3)),
                    sort(sample(pareto.rns, 1000, replace = T)))
...
if(Npdf >= K) chosenMarginals = marginals[sample(Npdf, K, replace = T)] else
  chosenMarginals =
  marginals[c(1L : Npdf, sample(Npdf, K - Npdf, replace = T))]
...
result = SimJoint::SJpearson(
  X = marginals, cor = targetCor, stochasticStepDomain = c(0, 1),
  errorType = "maxRela", seed = 456, maxCore = 7, convergenceTail = 8)
